...
/** Light section: the four pillars of every shop we build, as an asymmetric grid. */
export async function Pillars() {
  const t = await getTranslations("Webshops");

  return (
    <Section theme="light" id="pijlers">
      <SectionIntro eyebrow={t("pillars.eyebrow")} title={t.rich("pillars.title", richTags)} lead={t("pillars.intro")} />
        ...
        {ITEMS.map((k, i) => {
          ...
          return (
            <RevealItem key={k} as="article" className={cn("flex h-full flex-col bg-bg p-7 sm:p-9 lg:p-11", SPANS[i])}>
              <h3 className="text-h3 max-w-[18ch] text-balance">{t(`pillars.items.${k}.title`)}</h3>
